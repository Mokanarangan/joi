...
internals.root = function () {
    ...
    root.boolean = root.bool = function () {
        ...
        return internals.boolean.keys.apply(internals.boolean, arguments);
    };

    root.binary = function () {

        Hoek.assert(arguments.length === 0, 'Joi.binary() does not allow arguments.');
        internals.binary._code = code;

        return internals.binary;
    };

    root.date = function (code) {

        // Hoek.assert(arguments.length === 0, 'Joi.date() does not allow arguments.');
        internals.date._code = code;

        return internals.date.keys.apply(internals.date, arguments);
    };
    ...
    root.number = function (code) {
        return internals.number.keys.apply(internals.number, arguments);
    ...
    root.string = function (code) {

        // Hoek.assert(arguments.length === 0, 'Joi.string() does not allow arguments.');
        internals.string._code = code;
        return internals.string.keys.apply(internals.string, arguments);
    };
